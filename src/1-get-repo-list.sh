...
if ! [ "$REPO_LIST" == "" ]; then
    echo overwriting repo list from $REPO_LIST
    cat "${REPO_LIST}" > src/1-repo-list.txt

    echo "pushd repos" > src/1.1-clone.sh
    cat "${REPO_LIST}" | awk '{print "git clone "$1}' >> src/1.1-clone.sh
    echo "popd" >> src/1.1-clone.sh

    echo "pushd repos" > src/1.1-pull.sh
    cat "${REPO_LIST}" | cut -d'/' -f2 | sed 's/.git//g' | awk '{print "pushd "$1"; git pull; popd"}' >> src/1.1-pull.sh
